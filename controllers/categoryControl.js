  ...
  try {
    ...
    if (categoryname) {
      let findData = await productModel
        ...
        .populate([
          { path: "subCategorySchema" },
          { path: "extraCategorySchema" },
        ]);
...
const addProduct = async (req, res) => {
  try {
    const { title, category, subcategory, extracategory, price, discount } =
      req.body;
    let image = req.file ? req.file.path : "";

    let extraProductData = await extraCategoryModel.create({
      title,
      price,
      discount,
      image,
    });
    let subProductData = await subCategoryModel.create({
      category,
      subcategory,
      extraCategorySchema: extraProductData._id,
    });
    await productModel.create({
      category,
      subcategory,
      extracategory,
      subCategorySchema: subProductData._id,
      extraCategorySchema: extraProductData._id,
    });
...
const getcate = async (req, res) => {
  let updateSub = await productModel
    .find()
  ...
  try {
    ...
    if (findemail) {
      const transporter = await mailer.createTransport({
        service: "gmail",
        auth: {
          user: "priyanshumishra7272@gail.com",
          pass: "",
        },
      });

      const sendMail = {
        from: "priyanshumishra7272@gail.com",
        to: email,
